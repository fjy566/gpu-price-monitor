...
| 失败型号可单独重试 | 自定义型号可添加/删除 | 数据新鲜度筛选与 CSV 导出 |
| 采集结束自动退出浏览器 | 开始前自动保存采集范围 | 实时日志和本轮采集摘要 |
...
| 推荐区域 | 基于市场基准的低价候选 |
| 采集日志 | 实时查看成功、过滤、无数据和异常原因，失败型号可单独重试 |
...
| `POST /api/control/retry_failed` | 只重试上一轮失败型号 |
...
先点击“打开闲鱼登录”，扫码后再点击“校验登录”。登录态保存在项目专用 profile 中，不等同于日常 Chrome 的登录态。
...
升级或清理数据前，可以先复制 `prices.db` 做备份。不要把 `.sessions/` 或 `.chrome_profile/` 上传到公共仓库。Web API 默认只允许本机客户端；确需局域网访问时，需显式设置 `GPU_MONITOR_ALLOW_REMOTE=1`，并自行增加鉴权和防火墙规则。
